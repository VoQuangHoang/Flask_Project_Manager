...
class Role(db.Model):
    ...
    update_at = db.Column(db.TIMESTAMP,server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), nullable=False)
...
class User(UserMixin,db.Model):
    ...
    tbl_user_device_history = db.relationship("User_device_history", back_populates="tbl_user",cascade="all, delete",passive_deletes=True)
    create_at = db.Column(db.TIMESTAMP, nullable=False, server_default=db.func.now())
    update_at = db.Column(db.TIMESTAMP,server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), nullable=False)
...
class User_device_history(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('tbl_user.user_id', ondelete="CASCADE"), nullable=False)
